  ...
  return (
    <div className={variant === 'dark' ? 'bg-navy-900' : 'bg-steel-50 border-y border-steel-200'}>
      <div className="container-8xl py-4 md:py-5">
        <div className="grid grid-cols-2 items-center gap-x-2 gap-y-3 sm:gap-x-4 lg:grid-cols-4 lg:gap-4">
          {items.map((item) => {
            const Icon = ICONS[item.icon] ?? BadgeCheck;
            return (
              <div key={item.label} className="flex min-w-0 items-center justify-center gap-1.5 sm:gap-2">
                ...
                <span className={`min-w-0 whitespace-nowrap text-[7px] font-semibold leading-tight min-[360px]:text-[8px] sm:text-[11px] md:text-xs lg:text-sm ${variant === 'dark' ? 'text-navy-50' : 'text-navy-700'}`}>
                  {item.label}
                </span>
              </div>
            );
